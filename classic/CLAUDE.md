# CLAUDE.md — classic (Carta 6.18.x)

**This directory is frozen.** `classic/index.html` is the third turn of CARTA,
kept whole and running as a museum with the lights on: **no fixes, no
features, no refactors.** Anything classic appears to "need" is either a
Carta 7 feature or nothing (`docs/ROADMAP.md`, and the outfitter skill's
cross-repo section). Do not edit files here unless the founder asks for
something in classic *by name*.

What follows is the architecture map of that app, moved here whole when the
root `CLAUDE.md` was rewritten for Carta 7. It is kept because the third turn
was a serious piece of work and its reasoning — the resolver, the pen, the
strike, the map's four laws — is the record of *why* Carta 7 refuses what it
refuses. `docs/PIVOT.md` §10 is the summary; this is the detail.

**Reading it:** every unqualified path below is written from the repo root as
it stood *before* the turn. Read `index.html` as **`classic/index.html`**, and
read the `docs/*.md` specs it cites (`RESOLVER.md`, `GEOCODE.md`, `MAPPING.md`,
`CORRECTIONS.md`, `READER.md`, `SURFACES.md`, `VISION.md`, `SCHEMA.md`,
`PLATFORM.md`, `MODEL_QA.md`, `LOT_IDENTITY.md`) as the third turn's design
record — still in `docs/`, still accurate about classic, and superseded for
Carta 7 by `docs/PIVOT.md`, `docs/ROADMAP.md` and `docs/ARCHITECTURE.md`.
`server/` is likewise classic's — dormant, not part of Carta 7.

The one thing here that is **not** history: `docs/VOICE.md` still governs every
user-facing string in both apps.

---

## What classic is

*(Written in the present tense of the third turn — CARTA was this app, and
inside `classic/` it still is.)*
...
## Repository layout (as it stood before the turn)
...
## The app (`index.html` — now `classic/index.html`)
...
## Running & testing (classic)

**The app:** open `classic/index.html` in a browser, or serve the repo
statically (e.g. `python3 -m http.server`) and visit it. No build. It is
linked from Carta 7's Shelf as **Open classic**.

**The server** (dormant — kept for the record and for the horizon's backup
option, not deployed as part of Carta 7):
...
If the server is ever woken up: **an API change lands in both `server.js` and
`worker.mjs`, and `node server/test.js` + `node server/test-worker.js` both
stay passing.** The two servers must stay in lockstep.

Classic's frontend has no automated test harness; it was verified by loading
the page. That stance is unchanged, because the file is unchanged.
...
production — behind fly.io, Caddy, nginx, or Tailscale Funnel.
`http://localhost` is the one exception browsers allow, so local dev needs
no TLS.

## Conventions (classic, for the record)

- **The terse code style.** Compact single-line helpers and inline handlers in
  the frontend; idiomatic, commented Node with section banners in the server.
- **Bundle-free and single-file**, both halves. Classic's network reads are all
  *optional* progressive enhancements that degrade offline: the café address
  lookup (Nominatim → manual text), the origin map rail (`placeSearch`: Photon
  → Nominatim → typed text), the brand read (`readBrand`: Microlink → the
  site's palette, name and description), and the street map (MapLibre GL +
  OpenFreeMap vector tiles, lazy-loaded → the drawn plot). Keyless,
  accountless, bundling nothing.
- **`docs/VOICE.md` governs every user-facing string** — the one live law in
  this file.
