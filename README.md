...
- **The atlas, drawn on the map.** Chart No. 1 draws as a map, folded into the same atlas surface — a **Map / List** toggle over the same greens. The season's origins sit at a coarse-but-honest position, the roasters in their city, the bars on their streets, with a thin line tracing a green from origin to roaster to bar. Every mark taps into its page — a grower, a roaster, a venue — so the map and the pages are one walk, and a stranger reads the whole scene read-only. Coordinates stay honest: a venue at its real point, a roaster at its city, a green at its region — coarse where the record is coarse, never a farm-precise pin invented, and a node with no coordinate lists rather than lies. The streets are an enhancement: the drawn plot renders first from saved positions and stands alone offline, the whole scene working — pins, taps, lines — with no tiles at all. Author a roast online and its city and origin place themselves, from the same keyless lookup the café address uses.
